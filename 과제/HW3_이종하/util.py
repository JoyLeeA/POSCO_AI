...
class LinkedList(tuple):
    ...
    def construct(self, el):  # construct a new list by append an element. (corresponding to Lisp's cons)
        return LinkedList((el, self))

    @property
    def value(self):  # return the head of the list. (corresponding to Lisp's car)
        return self[0] if self is not LinkedList.NIL else self

    @property
    def tail(self):  # return the remaining list where the head is excluded. (corresponding to Lisp's cdr)
