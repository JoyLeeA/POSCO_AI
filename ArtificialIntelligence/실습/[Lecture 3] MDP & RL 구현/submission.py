...
class ExampleMDP(util.MDP):
    ...
    def actions(self, state): # Action들의 List 반환
        return ['Left', 'Right']
    ...
    def succAndProbReward(self, state, action):
        ...
        if action == 'Left': #각 action에 대해 튜플 추가
        ...
        return results #result tuple 반환
...
class BlackjackMDP(util.MDP):
    def __init__(self, cardValues, multiplicity, threshold, peekCost):
        ...
        """
        self.cardValues = cardValues #각각의 카드 표현 [1, 3, 5]
        self.multiplicity = multiplicity # 각각의 카드가 몇장?
        self.threshold = threshold #최대 한도
    ...
    def succAndProbReward(self, state, action): # state에서 action을 취하면 얻을 수 있는 후보들
        # BEGIN_YOUR_CODE
        ...
        if deck is None:  # when there is no card in the deck
            ...
            def get_succ_reward(idx): #어떠한 idx를 넣었을때(뽑았) 다음 state와 reward를 출력해주는 함수
        ...
        #card_sum: 현재 가지고 있는 카드의 합, 항상 똑같이 종료하기 때문에 1
        else:
...
class QLearningAlgorithm(util.RLAlgorithm):
    def __init__(self, actions, discount, featureExtractor, explorationProb=0.2):
        ...
        self.weights = defaultdict(float) # 학습으로 얻고자 하는 것
        self.numIters = 0
    ...
    def getAction(self, state):
        self.numIters += 1
        if random.random() < self.explorationProb: #0.2의 확률로 랜덤 액션을 취하겠다.
            return random.choice(self.actions(state))
        else:
            return max((self.getQ(state, action), action) for action in self.actions(state))[1]

    # Call this function to get the step size to update the weights.
    def getStepSize(self): # 에타를 의미, 시간이 지날수록 줄어든다.
