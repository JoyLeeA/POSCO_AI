  ...
  def getAction(self, gameState):
    ...
    if player == 0:
        ...
        for action in legalMoves:
          ...
          values.append(value)
          alpha = max(alpha, value)
        return max(zip(legalMoves, values), key=lambda x: x[1])[0]
...
def betterEvaluationFunction(currentGameState):
  ...
  *** defined features ***
  ...
  score += _FILL_IN_  # HINT: use defined features (e.g. score += - 0.3 * depth_food if depth_food else 0)
